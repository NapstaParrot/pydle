...
def start(*prev_guess) :
    global guesses; guesses = 0
    global guess; guess = []
    global guess_color; guess_color = []
...
def draw(*err) :
    ...
    if (game_state == "win") :
        print(f"{green}Congratulations, you found the answer in {guesses} attempt{'s' if guesses > 1 else ''}!\n" + white)
...
def help_page() :
    while 1 :
        ...
        if (inp == "1") :
            ...
            print("A mystery word is chosen when the program starts. Its your job to find that mystery word.")
            print("You have 6 total tries. You must enter a valid 5 letter word. Said word will give you color")
            print("indentification at the side. \n\nFor example :")

            print(bold + f"c r a t e  |  {green}0 {gray}0 0 0 0", white)
            print("This means that the letter c is at the correct spot.\n")

            print(bold + f"t r e e s  |  {gray}0 0 {yellow}0 {gray}0 0", white)
            print("This means that one of the two letter e is in the word, but isn't at the correct spot.\n")

            print("And if the letter is gray, it means that the letter isn't in the word.\n")

            input(gray + "[Press enter to continue]" + white)
            ...
            print("your next guess. Try to get the most wordle possible.\n")
